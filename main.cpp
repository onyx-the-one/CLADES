...
static void param_panel()
{
    ...
    ImGui::SliderDouble("beta (contact rate)", &g.params.beta, 0.0, 2.0, "%.4f");
    ImGui::SliderDouble("sigma (1/incubation)", &g.params.sigma, 0.01, 1.0, "%.4f");
    ImGui::SliderDouble("gamma (1/infect. period)", &g.params.gamma, 0.01, 1.0, "%.4f");
    ...
    ImGui::SliderDouble("IFR (fatality ratio)", &g.params.ifr, 0.0, 0.3, "%.4f");
    ImGui::SliderDouble("Hosp. rate (frac I)", &g.params.hosp_rate, 0.0, 0.5, "%.4f");
    ImGui::SliderDouble("delta (hosp. extra mort)", &g.params.delta, 0.0, 0.1, "%.4f");

    ImGui::Spacing();
    ImGui::TextUnformatted("Immunity & Vaccination");
    ImGui::Separator();

    ImGui::SliderDouble("omega_r (wane recov/day)", &g.params.omega_r, 0.0, 0.05, "%.5f");
    ImGui::SliderDouble("omega_v (wane vax/day)",  &g.params.omega_v, 0.0, 0.05, "%.5f");
    ImGui::SliderDouble("Vax rate (frac S/day)",   &g.params.vax_rate, 0.0, 0.05, "%.5f");
    ImGui::SliderDouble("Vax efficacy",            &g.params.vax_eff,  0.0, 1.0,  "%.3f");
    ImGui::SliderDouble("Vax coverage cap",        &g.params.vax_cov,  0.0, 1.0,  "%.3f");

    ImGui::Spacing();
    ImGui::TextUnformatted("Seasonality");
    ImGui::Separator();

    ImGui::SliderDouble("Season amplitude", &g.params.season_amp, 0.0, 1.0, "%.3f");
    ImGui::SliderDouble("Season phase (days)", &g.params.season_phi, 0.0, 365.0, "%.1f");

    ImGui::Spacing();
    ImGui::TextUnformatted("Stochastic Noise (OU on beta)");
    ImGui::Separator();

    ImGui::SliderDouble("Noise sigma", &g.params.noise_sigma, 0.0, 0.5, "%.4f");
    ImGui::SliderDouble("Noise theta (mean-rev)", &g.params.noise_theta, 0.0, 1.0, "%.4f");
    ...
    ImGui::SliderDouble("dt (step, days)", &g.params.dt, 0.01, 1.0, "%.3f");
}
